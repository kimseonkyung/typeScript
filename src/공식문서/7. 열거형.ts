 ...
 *
 * 열거형을 사용하는 것은 간단합니다
 * 그냥 열거형 자체에서 프로퍼티로 모든 멤버에 접근하며,
 * 열거형의 이름을 사용해 타입을 선언합니다.
 */
 ...
 * "직렬화"를 잘한다는 이점이 있습니다. 다시 말해서 만약 당신이 숫자 열거형을 이용해서
 * 디버깅하고 있고 그 값을 읽어야 한다면, 종종 그 값이 불확실한 경우가 있습니다
 ...
 * 리터럴 열거형 멤버 리터럴 열거형 멤버는 초기화 값이 존재하지 않거나,
 * 아래 값들로 초기화되는 멤버입니다.
 *
 ...
 * 아직 유니언 타입에 대해서 배우진 않았지만,
 * 유니언 타입 열거형을 사용하면 타입 시스템이 열거형 자체에 존재하는
 * 정확한 값의 집합을 알고 있다는 사실을 활용할 수 있다는 점만 알아두면 됩니다.
...
/**
 * 위 코드에서 x 가 E.Foo 가 아닌지 확인합니다.
 * 만약 이 조건이 true 라면, || 조건은 더는 체크할 필요가 없으므로
 * if 아래의 body가 실행될 것입니다. 그러나 만약 이 조건이 통과되지 않는다면,
 ...
 *
 * 열거형이 런타임에 존재하는 실제 객체라고 할지라도,
 * keyof 키워드는 일반적인 객체에서 기대하는 동작과 다르게 동작합니다.
 ...
 *
 * 숫자 열거형 멤버는 멤버의 프로퍼티 이름을 가진 객체를 생성하는 것 외에도
 * 열거형 값에서 열거형 이름으로 역 매핑 을 받습니다.
 */
...
/**
 * 이렇게 생성된 코드에서, 열거형은 정방향 (name -> value) 매핑과
 * 역방향 (value -> name) 매핑 두 정보를 모두 저장하는 객체로 컴파일됩니다.
 ...
 * 열거형 값에 접근할 때, 추가로 생성된 코드 및 추가적인 간접 참조에 대한
 * 비용을 피하기 위해 const 열거형을 사용할 수 있습니다.
...
/**
 * const 열거형은 상수 열거형 표현식만 사용될 수 있으며 일반적인 열거형과 달리
 * 컴파일 과정에서 완전히 제거됩니다. const 열거형은 사용하는 공간에 인라인됩니다.
...
/**
 * ambient 열거형과 비-ambient 열거형의 가장 큰 차이점은,
 * 일반적인 열거형에서 초기화되지 않은 멤버가 상수로 간주하는 멤버 뒤에 있다면,
 * 이 멤버도 상수로 간주할 것입니다. 반면 (const가 아닌) ambient 열거형에서
 * 초기화되지 않은 멤버는 항상 계산된 멤버로 간주합니다.
 */
